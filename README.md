...
cd tests/roms/spike && ./build.sh             Haxe to a ROM
```
...
Requires Haxe 4.3+, `git` and `curl`. Images, palettes and pattern sets are compiled by `hxres`
itself, and padding and checksums by `haxelib run hx68k pad`, so neither needs a JVM. What still
does is a sprite and a tune: `rescomp.jar` cuts the one and calls the native `xgmtool` for the
other, and both are being replaced. A build opens SGDK for the m68k toolchain and nothing else, so
...
tests/roms/     art, bench, bug, conformance, events, hardware, sdk, sound, spike
